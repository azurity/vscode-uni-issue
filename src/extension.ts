...
export function activate(context: vscode.ExtensionContext) {
	...
	let exportDisposable = vscode.commands.registerCommand('uni-issue.export', async () => {
	...
	let removeDisposable = vscode.commands.registerCommand('uni-issue.remove', async (item: Issue) => {
	...
	let finishDisposable = vscode.commands.registerCommand('uni-issue.finish', async (item: Issue) => {
		let key = item.$meta.base + "/.vscode/.uni_issue.yaml";
		if (issueSet.get(key)) {
			let index = issueSet.get(key)!.issues.findIndex((v) => v == item);
			if (index >= 0) {
	...
	let unfinishDisposable = vscode.commands.registerCommand('uni-issue.unfinish', async (item: Issue) => {
		let key = item.$meta.base + "/.vscode/.uni_issue.yaml";
		if (issueSet.get(key)) {
			let index = issueSet.get(key)!.issues.findIndex((v) => v == item);
			if (index >= 0) {
...
type SpaceSet = { base: vscode.Uri, watcher: vscode.FileSystemWatcher, issues: Issue[] };
...
async function reloadWorkspaceInfo() {
	...
	if (vscode.workspace.workspaceFolders !== undefined) {
		for (let ws of vscode.workspace.workspaceFolders) {
			...
			let watcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(uri, '.uni_issue.yaml'));
			...
			try {
				issueSet.get(key)!.issues = await loadYaml(vscode.Uri.joinPath(uri, '.uni_issue.yaml'));
			} catch (e) { }
			refreshHighlight();
		}
	}
...
async function storeYaml(uri: vscode.Uri, data: any[]) {
	await vscode.workspace.fs.writeFile(uri, Buffer.from(YAML.stringify(data), 'utf-8'));
}
...
function createIssueDialog(base: vscode.Uri, key: string, file: string, region: number[]) {
	let panel = vscode.window.createWebviewPanel('markdown.preview', 'create issue',
...
<html>
<head>
<link href="${panel.webview.asWebviewUri(vscode.Uri.file(path.resolve(__dirname, '../static/webview.css')))}" rel="stylesheet">
</head>
<body>
<script>const formDesc = ${JSON.stringify(desc)};</script>
<script src=${panel.webview.asWebviewUri(vscode.Uri.file(path.resolve(__dirname, '../static/webview.js')))}></script>
</body>
</html>
	`;
			...
			if (base == docPath.slice(0, base.length)) {
				...
				for (let item of issueSet.get(k)!.issues) {
					...
					if (meta.file == subPath) {
						if (!meta.status) {
							decos.push(new vscode.Range(meta.region[0], meta.region[1], meta.region[2], meta.region[3]));
						} else {
							decosFin.push(new vscode.Range(meta.region[0], meta.region[1], meta.region[2], meta.region[3]));
						}
					}
				}
		...
		for (let item of issueSet.get(k)!.issues) {
			...
			for (let key of head) {
				if (item[key] === undefined) {
					...
					result[key] = item[key];
					if (result[key][0] == '$') {
						if (specialValueExplain[result[key]] && item['$meta'] !== undefined) {
...
class IssueTreeProvider implements vscode.TreeDataProvider<SpaceSet | Issue> {
	private _onDidChangeTreeData = new vscode.EventEmitter<void | SpaceSet | Issue | (SpaceSet | Issue)[] | null | undefined>();
	onDidChangeTreeData: vscode.Event<void | SpaceSet | Issue | (SpaceSet | Issue)[] | null | undefined> = this._onDidChangeTreeData.event;

	referesh() {
		this._onDidChangeTreeData.fire();
	}
	getTreeItem(element: SpaceSet | Issue): vscode.TreeItem | Thenable<vscode.TreeItem> {
			...
			return {
				...
				command: {
					...
					arguments: [
						...
						{
							selection: new vscode.Range(meta.region[0], meta.region[1], meta.region[2], meta.region[3]),
	...
	getChildren(element?: SpaceSet | Issue): vscode.ProviderResult<(SpaceSet | Issue)[]> {
		...
		for (let k of issueSet.keys()) {
			...
			if (base == docPath.slice(0, base.length)) {
				...
				for (let item of issueSet.get(k)!.issues) {
					let meta = item["$meta"];
					if (meta.file == subPath) {
						let range = new vscode.Range(meta.region[0], meta.region[1], meta.region[2], meta.region[3]);
